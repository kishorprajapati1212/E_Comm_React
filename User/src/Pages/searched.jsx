...
const Searched = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const result = await axios.get(`${Backend_url}/product_name`);
            ...
            {searchQuery && suggestions.length > 0 && !suggestionClicked && (
                <Paper component={List} elevation={3} sx={{ zIndex: 9999, position: 'absolute' }}>
                    {suggestions.map((suggestion, index) => (
                        <ListItem button key={index} onClick={() => handleSuggestionClick(suggestion)} sx={{zIndex:2}}>
                            <ListItemText primary={suggestion} sx={{ borderBottom: "1px solid white",zIndex:1 }} />
                        </ListItem>
                    ))}
                </Paper>
            )}
